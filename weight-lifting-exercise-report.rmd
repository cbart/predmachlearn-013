...
For fitting the model we are going to use the `caret` package. We are going to try different machine learning methods evaluating them using repeated 10-fold cross validation.
...
rfFit <- train(classe ~ ., data = tr, method = "rf", trControl = fitControl, verbose = FALSE)
gbmFit <- train(classe ~ ., data = tr, method = "gbm", trControl = fitControl, verbose = FALSE)
earthFit <- train(classe ~ ., data = tr, method = "bagEarth", trControl = fitControl)
treebagFit <- train(classe ~ ., data = tr, method = "treebag", trControl = fitControl)
ldaFit <- train(classe ~ ., data = tr, method = "lda", trControl = fitControl)
```

Method | Accuracy on 10-fold cross validation
--- | ---
Random Forests `rf` | 84%
...
Random Forests and Stochastic Gradient Boosting seem to perform the best on given training data. Both perform more or less the same in terms or accuracy (at levels of 84%).